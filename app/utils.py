...
class PatternTokenizer(object):
    ...
    def __init__(self, lower=True, initial_filters=r"[^a-z0-9!@#\$%\^\&\*_\-,\.' ]", re_path='lib/re_patterns.json',
...
class toxic_ensemble(object):
    ...
    '''
    def __init__(self, 
                 embeddings_path = r'lib/glove.6B.300d.txt',       # If model is trained, don't worry about this. 
        ...
        if model_path is None:  # initialize if no model path
            # Initialize TFIDF
        ...
        else:   # load pretrained tokenizers and models if exist
            self.classic_m, self.r = load(model_path+'/classic_m')
            self.nn_m = tf.keras.models.load_model(model_path+'/nn_m')
            self.keras_tokenizer = load(model_path+'/keras_tokenizer')
            self.tfidf = load(model_path+'/tfidf')
